...
# AdamW replacement for the block matrix parameters that Muon optimizes in the baseline.
# The auxiliary AdamW parameter groups below are intentionally left unchanged.
block_adamw_lr = 0.0015
block_adamw_weight_decay = 0.10
block_adamw_warmup_steps = 250
block_adamw_betas = (0.9, 0.95)
...
optimizer2 = AdamW([p for p in model.blocks.parameters() if p.ndim >= 2],
                   lr=block_adamw_lr, betas=block_adamw_betas, eps=1e-10,
                   weight_decay=block_adamw_weight_decay, fused=True)
...
for opt in optimizers:
    for group in opt.param_groups:
        ...
        group["warmup_steps"] = block_adamw_warmup_steps

wandb_run = None
if dist.get_rank() == 0:
    wandb_run = wandb.init(
        project=os.environ.get("WANDB_PROJECT", "nanogpt-fingerprinting"),
        name='entry_02_20260426_adam_lr_0_0015_betas_0_9_0_95_warmup_a63a68d1',
    )
...
fingerprint = OptimizerFingerprint.attach(
    model,
    optimizers,
    run_name='entry_02_20260426_adam_lr_0_0015_betas_0_9_0_95_warmup_a63a68d1',
